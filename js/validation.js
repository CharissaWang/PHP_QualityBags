        ...
        success: function (result) {
            switch (result)
            {
                ...
                case 'disabled':
                    ...
                    break;
                default:
                    if (txtEmail.value == 'admin@qualitybags.co.nz')
                    {
                        window.location.href = "../admin/Admin.php?page=BagManagement";
                    }
                    else
                    {
                        document.getElementById("frmMain").submit();
...
/**
 * Input validation when add new bags.
 * @returns {boolean}
 * @constructor
 */
function ValidateAddBag() {
    var bagName = document.getElementById("txtBagName");
    var bagCategory = document.getElementById("ddlCategory");
    var bagSupplier = document.getElementById("ddlSupplier");
    var bagPrice = document.getElementById("txtPrice");
    var bagDescription = document.getElementById("txtDescription");

    if (IsEmpty(bagName) || IsEmpty(bagCategory) || IsEmpty(bagSupplier) || IsEmpty(bagPrice) || IsEmpty(bagDescription)) {
...
/**
 * Submit add bag form after validation
 * @returns {boolean}
 */
function btnSaveBag_OnClick() {
    if (!ValidateAddBag()) {
        return false;
    }
    else {
...
function btnSaveSupplier_OnClick() {
    ...
    $.ajax({
        url: "/wangc95/php_assignment/admin/AddSupplierProcess.php",
        type: "POST",
        data: {
...
function btnSaveCategory_OnClick() {
    ...
    $.ajax({
        url: "/wangc95/php_assignment/admin/AddCategoryProcess.php",
        type: "POST",
        data: {
