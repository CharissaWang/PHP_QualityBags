...
class Business
{
    ...
    {
        return $this->data->getBagByID($bagID);
    }

    /**
     * Get bag details by bag ID
     * @param $bagID
     * @return bool|mysqli_result
     */
    function getBagDetails($bagID)
    {
        return $this->data->getBagDetails($bagID);
    }
    ...
    {
        return $this->data->getBags($from, $count);
    }
    ...
    function getBagsByCategory($categoryID, $from, $count)
    {
        ...
        {
            return $this->data->getBagsByCategory($categoryID, $from, $count);
        }
    }

    /**
     * Get total quantity of all the bags
     * @return int
     */
    function getBagsQuantity()
    {
        return mysqli_num_rows($this->data->getAllBags());
    }

    /**
     * Get total quantity of bag of a given category
     * @param $categoryID
     * @return int
     */
    function getBagsQuantityByCategory($categoryID)
    {
        if ($categoryID == 'All')
        {
            return $this->getBagsQuantity();
        }
        else
        {
            return mysqli_num_rows($this->data->getBagsByCategory($categoryID));
        }
    }

    /**
     * Get the price of a given bag
     * @param $bagID
     * @return mixed
     */
    function getBagPrice($bagID)
    {
        $bags = $this->data->getBagByID($bagID);
    ...
    /**
     * Save a new bag to the database
     * @param $bagName
     * @param $supplierID
     * @param $categoryID
     * @param $price
     ...
     * @return bool|mysqli_result
     */
    function addBag($bagName, $description, $price, $imagePath, $categoryID, $supplierID)
    {
        return $this->data->insertBag($bagName, $description, $price, $imagePath, $categoryID, $supplierID);
    }
    ...
    /**
     * Save bag ID and quantity of an order to database
     * @param $orderID
     * @param $bagID
     * @param $qty
     * @return bool|mysqli_result
     */
    function createOrderItem($orderID, $bagID, $qty)
    {
        return $this->data->insertOrderItem($orderID, $bagID, $qty);
    }
